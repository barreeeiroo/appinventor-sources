...
public class AndroidManifest implements Task {
  ...
  public TaskResult execute(ExecutorContext context) {
    ...
    String vCode = (context.getProject().getVCode() == null) ? YoungAndroidValues.DEFAULT_VERSION_CODE : context.getProject().getVCode();
    String vName = (context.getProject().getVName() == null) ? YoungAndroidValues.DEFAULT_VERSION_NAME : cleanName(context.getProject().getVName());
    if (context.isIncludeDangerousPermissions()) {
      vName += "u";
    }
    String aName = (context.getProject().getAName() == null) ? YoungAndroidValues.DEFAULT_APP_NAME : cleanName(context.getProject().getAName());
    ...
    try {
      ...
      int minSdk = Integer.parseInt((context.getProject().getMinSdk() == null) ? YoungAndroidValues.DEFAULT_MIN_SDK : context.getProject().getMinSdk());
      if (!context.isForCompanion()) {
